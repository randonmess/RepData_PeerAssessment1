...
The mean and median of the total steps per day are `r format(meansteps, scientific = F)` and `r mediansteps` respectively.
...
The `r maxinterval`-th interval contains the maximum number of steps, averaged across all days.
...
The mean and median of the total steps per day are `r format(newmeansteps, scientific = F)` and `r format(newmediansteps, scientific = F)` respectively. Since each interval with an NA value has been imputed with the average interval value across all days, the mean total remains the same, but there is an significant increase in the number of dates that have the average total, thus the median has shifted towards the mean value.
